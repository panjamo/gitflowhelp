...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Add { branch, description, commit, push, force } => add_or_modify_description(branch, description, false, commit, push, force),
        Commands::Modify { branch, description, commit, push, force } => add_or_modify_description(branch, description, true, commit, push, force),
        Commands::List { detailed, all } => list_descriptions(detailed, all),
    }
}

fn add_or_modify_description(target_branch: Option<String>, description: Option<String>, is_modify: bool, commit: bool, push: bool, force: bool) -> Result<()> {
    ...
    if !is_current_branch {
        ...
        if !force {
            println!("⚠️  WARNING: You are about to modify branch '{}' while on '{}'", target_branch, current_branch);
            ...
            println!("🚀 Force mode: Modifying branch '{}' without confirmation", target_branch);
        }
    }
    ...
    if is_current_branch {
        ...
        if !commit && !push {
            println!("⚠️  WARNING: Description prepared for branch '{}' but not committed.", target_branch);
    ...
    println!("✅ {} description for branch '{}'", action, target_branch);
    ...
    for branch_result in local_branches {
        ...
        if let Some(branch_name) = branch.name()? {
            ...
            if let Ok(description) = read_branch_description_from_git(&repo, branch_name) {
                if !description.is_empty() {
                    found_any = true;
                    process_branch_description(branch_name, &description, detailed, &mut branch_descriptions)?;
                } else if show_all {
                    found_any = true;
...
fn commit_current_branch_changes(repo: &Repository, branch_name: &str, is_modify: bool, push: bool) -> Result<()> {
    // Stage the BRANCHREADME.md file
    let mut index = repo.index()
        .context("Failed to get repository index")?;
    index.add_path(Path::new("BRANCHREADME.md"))
    ...
    let commit_message = format!("{} branch description [skip ci]", 
...
fn commit_to_branch(repo: &Repository, branch_name: &str, description: &str, is_modify: bool, push: bool) -> Result<()> {
    ...
    if repo.state() != git2::RepositoryState::Clean {
        println!("⚠️  WARNING: Repository has uncommitted changes. These will not interfere with the remote branch operation.");
    }
    
    // Get the current commit
    let target_commit_oid = branch_ref.target()
        .context("Failed to get target commit")?;
    let target_commit = repo.find_commit(target_commit_oid)
        .context("Failed to find commit")?;
    
    // Get the current tree
    let current_tree = target_commit.tree()
        .context("Failed to get tree from commit")?;
    
    // Create a new blob with the description content
    let blob_oid = repo.blob(description.as_bytes())
    ...
    tree_builder.insert("BRANCHREADME.md", blob_oid, 0o100644)
        .context("Failed to insert BRANCHREADME.md into tree")?;
    let new_tree_oid = tree_builder.write()
        .context("Failed to write new tree")?;
        ...
        Err(_) => {
            ...
            if branch_name.contains('/') {
                anyhow::bail!(
                    ...
                    branch_name, 
                    branch_name.split('/').last().unwrap_or(branch_name),
                    branch_name
                );
            } else {
                anyhow::bail!(
...
fn read_branch_description_from_git(repo: &Repository, branch_name: &str) -> Result<String> {
    ...
    } else if branch_name.starts_with("origin/") || branch_name.starts_with("upstream/") || 
              (branch_name.contains("/") && repo.find_reference(&format!("refs/remotes/{}", branch_name)).is_ok()) {
    ...
    let commit_oid = branch_ref.target()
        .context("Failed to get target commit")?;
